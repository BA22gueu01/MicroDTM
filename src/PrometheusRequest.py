...
class PrometheusRequest:
    ...
    def makeRequest(self, requestParam):
        ...
        if requestParam == "uptime":
            prometheusResponse = requests.get(self.PROMETHEUS + '/api/v1/query?query=uptime{container!~"istio-proxy", '
                                                                'kubernetes_namespace="sock-shop"}['
                                              + str(self.query_interval) + 'm:' + str(self.update_interval) + 'm]')
        elif requestParam == "uptime_history":
            request = '/api/v1/query?query=uptime{container!~"istio-proxy", kubernetes_namespace="sock-shop"}[' + \
                      str(self.historic_interval) + 'm:' + str(self.update_interval) + 'm]'
            ...
            cpuUsageCalculation = 'sum(rate(container_cpu_usage_seconds_total{container!~"istio-proxy", ' \
                                  'namespace="sock-shop"}[' + str(self.query_interval) + 'm:' \
                                  + str(self.update_interval) + 'm])) by (pod_name, container_name)/sum(' \
                                                                'container_spec_cpu_quota{container!~"istio-proxy", ' \
                                                                'namespace="sock-shop"}/container_spec_cpu_period{' \
                                                                'container!~"istio-proxy", namespace="sock-shop"}) by ' \
            ...
            # https://github.com/google/cadvisor/issues/2026
            cpuUsageCalculation = 'sum(rate(container_cpu_usage_seconds_total{container!~"istio-proxy", ' \
                                  'namespace="sock-shop"}[' + str(self.historic_interval) + 'm:' \
                                  + str(self.update_interval) + 'm])) by (pod_name, container_name)/sum(' \
                                                                'container_spec_cpu_quota{container!~"istio-proxy", ' \
                                                                'namespace="sock-shop"}/container_spec_cpu_period{' \
                                                                'container!~"istio-proxy", namespace="sock-shop"}) by ' \
        ...
        # https://brian-candler.medium.com/interpreting-prometheus-metrics-for-linux-disk-i-o-utilization-4db53dfedcfc
            ...
            diskReadCalculation = '(node_disk_read_time_seconds_total / node_disk_reads_completed_total)[' + str(
                self.query_interval) + 'm:' + str(self.update_interval) + 'm]'
            ...
            diskReadCalculation = '(node_disk_read_time_seconds_total / node_disk_reads_completed_total)[' + str(
                self.historic_interval) + 'm:' + str(self.update_interval) + 'm]'
            prometheusResponse = requests.get(self.PROMETHEUS + '/api/v1/query', params={'query': diskReadCalculation})

        elif requestParam == "disk_write":
            diskWriteCalculation = '(node_disk_write_time_seconds_total / node_disk_writes_completed_total)[' + str(
                self.query_interval) + 'm:' + str(self.update_interval) + 'm]'
        ...
        # https://www.tigera.io/learn/guides/prometheus-monitoring/prometheus-metrics/
            ...
            memoryCalculation = '(node_memory_Active_bytes/ node_memory_MemTotal_bytes)[' + str(
                self.query_interval) + 'm:' + str(self.update_interval) + 'm]'
            ...
            responseCalculation = '(http_request_duration_seconds_sum{container!~"istio-proxy", ' \
                                  'kubernetes_namespace="sock-shop"} / http_request_duration_seconds_count{' \
                                  'container!~"istio-proxy", kubernetes_namespace="sock-shop"})[' \
                                  + str(self.query_interval) + 'm:' + str(self.update_interval) + 'm] '
            ...
            requestParam = requestParam.replace("_history",
                                                "") + '{container!~"istio-proxy", kubernetes_namespace="sock-shop"}[' + str(
                self.historic_interval) + 'm:' + str(self.update_interval) + 'm]'
            prometheusResponse = requests.get(self.PROMETHEUS + '/api/v1/query', params={'query': requestParam})

        else:
            requestParam = requestParam + '{container!~"istio-proxy", kubernetes_namespace="sock-shop"}[' \
                           + str(self.query_interval) + 'm:' + str(self.update_interval) + 'm]'
            prometheusResponse = requests.get(self.PROMETHEUS + '/api/v1/query', params={'query': requestParam})
