...
class PrometheusRequest:
    ...
    def makeRequest(self, requestParam):
        ...
        # https://brian-candler.medium.com/interpreting-prometheus-metrics-for-linux-disk-i-o-utilization-4db53dfedcfc
            ...
            diskReadCalculation = 'rate(node_disk_read_time_seconds_total{instance="' + self.PROMETHEUS.strip("http://") \
                                  + '",job="' + job + '"}[5m]) / rate(node_disk_reads_completed_total{instance="' + \
                                  self.PROMETHEUS.strip("http://") + '",job="' + job + '"}[5m]) '
            ...
            diskWriteCalculation = 'rate(node_disk_write_time_seconds_total{instance="' + self.PROMETHEUS.strip("http://") \
        ...
        # https://www.tigera.io/learn/guides/prometheus-monitoring/prometheus-metrics/
            ...
            memoryCalculation = 'node_memory_Active_bytes{instance="' + self.PROMETHEUS.strip("http://") + '", job="' \
                                + job + '"}/node_memory_MemTotal_bytes{instance="' + self.PROMETHEUS.strip("http://") \
                                + '", job="' + job + '"}'
