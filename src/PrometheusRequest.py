...
class PrometheusRequest:
    ...
    def makeRequest(self, requestParam):
        ...
        if requestParam == "uptime":
            ...
            prometheusResponse = requests.get(self.PROMETHEUS + '/api/v1/query?query=uptime{container!~"istio-proxy", kubernetes_namespace="sock-shop"}[' + str(self.historic_interval) + 'm:' + str(self.update_interval) + 'm]')
            ...
            # https://github.com/google/cadvisor/issues/2026
            cpuUsageCalculation = 'sum(rate(container_cpu_usage_seconds_total{container!~"istio-proxy", namespace="sock-shop"}[' + str(self.historic_interval) + 'm:' + str(self.update_interval) + 'm])) by (pod_name, container_name)/sum(container_spec_cpu_quota{container!~"istio-proxy", namespace="sock-shop"}/container_spec_cpu_period{container!~"istio-proxy", namespace="sock-shop"}) by (pod_name, container_name)'
            prometheusResponse = requests.get(self.PROMETHEUS + '/api/v1/query', params={'query': cpuUsageCalculation})
        ...
        # https://www.tigera.io/learn/guides/prometheus-monitoring/prometheus-metrics/
            ...
            responseCalculation = '(http_request_duration_seconds_sum{container!~"istio-proxy", kubernetes_namespace="sock-shop"} / ' \
                    'http_request_duration_seconds_count{container!~"istio-proxy", kubernetes_namespace="sock-shop"})[' + str(self.historic_interval) + 'm:' + str(self.update_interval) + 'm]'
            prometheusResponse = requests.get(self.PROMETHEUS + '/api/v1/query', params={'query': responseCalculation})
