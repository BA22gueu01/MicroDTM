...
class LogLevelCheck:
    ...
    def checkLoglevel(self):
        ...
        output = subprocess.check_output(["kubectl", "exec", "-n", "sock-shop queue-master-6bf76bbfc-4hcwf", "--", "microk8s.kubectl", "logs", "queue-master-6bf76bbfc-4hcwf", "--namespace=sock-shop", "--v=1", "|", "grep", "-iE", "'(warning|error|fatal)'", "|", "wc", "-l"])
        print(output)
        #output = 20
        # to be updated
        if output > 25:
